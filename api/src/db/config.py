...
def create_empty_tables():
    ...
    for data_type in FIELD_MAPPINGS.keys():
        ...
        if data_type == "dockets":
            columns.extend(
                ["docket_type VARCHAR", "modify_date TIMESTAMP", "title TEXT"]
            )
...
def create_indexes():
    ...
    for data_type in FIELD_MAPPINGS.keys():
        table_name = f"{data_type}_cache"
        conn.sql(
            f"CREATE INDEX IF NOT EXISTS idx_{data_type}_agency ON {table_name}(agency_code)"
        )
        conn.sql(
            f"CREATE INDEX IF NOT EXISTS idx_{data_type}_docket_id ON {table_name}(docket_id)"
        )
    logger.info("Indexes created successfully!")


def refresh_cache(
    agency_code: str, docket_id: str, data_type_enum: RegulationsDataTypes
):
    conn = get_connection()
    ...
    logger.info(
        f"Refreshing {data_type} cache for agency code: {agency_code} and docket id: {docket_id}"
    )
    table_name = f"{data_type}_cache"
    ...
    if result[0] == 0:
        raise HTTPException(
            status_code=404,
            detail=f"No records found for agency code: {agency_code} and docket id: {docket_id}",
        )
    ...
    new_count = conn.sql(
        f"SELECT COUNT(*) FROM {table_name} WHERE agency_code = '{agency_code}'"
    ).fetchone()[0]
    logger.info(f"Inserted {new_count} records into {table_name}")
    logger.info(
        f"Refreshed {data_type} cache for agency code: {agency_code} successfully!"
    )
