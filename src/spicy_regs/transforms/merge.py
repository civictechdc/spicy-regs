...
These operate on the *whole* staged dataset at once — writing per-agency staging
Parquet, merging those files, deduplicating by key (keeping the latest
``modify_date``), partitioning comments, and building the feed summary and
per-agency rollups. They run columnar / out-of-core in DuckDB, so unlike a
record-stream :class:`~spicy_regs.transforms.base.Transform` they cannot be
expressed as ``apply(records) -> records`` without buffering everything in
memory.

This module is the home of these transforms. The legacy
``spicy_regs.pipeline.transform`` module now re-exports from here for backward
compatibility while the ``pipeline`` package is being retired:
